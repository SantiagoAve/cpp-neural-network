...
/*
    LAYER CONSTRUCTOR:
*/
Layer::Layer(int input_size, int output_size, ActivationFunction activation_function,
...
Eigen::MatrixXd Layer::backward(const Eigen::MatrixXd & propag_loss_grad, double learning_rate) {
    ...
    derivative_z = propag_loss_grad.array() * this->activation_derivative(this->layer_z).array();
