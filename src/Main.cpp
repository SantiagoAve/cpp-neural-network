...
*/
int main() {
    // Let's create the optimizer first:
    auto sgd = std::make_unique<SGD>(0.4);
    ...
    Network network({layer_1, layer_2}, std::move(sgd));
    ...
    for (short i = 0; i < epochs; i++) {
        ...
        network.net_backward(test_true_val_1, test_pred_values_1);

        // Now, with another bach:
        Eigen::MatrixXd test_pred_values_2 = network.net_forward(test_input_2);

        network.net_backward(test_true_val_2, test_pred_values_2);
